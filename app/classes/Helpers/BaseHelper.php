...
Class BaseHelper
{
    ...
    public static function sendErrorMessage(int $chatID): void
    {
        Request::sendMessage([
            'chat_id' => $chatID,
            'text' => 'К сожалению произошла ошибка. Если есть возможность, свяжитесь с разработчиком и сообщите о проблеме.'
        ]);
    }

    /**
     * Статичный метод для отправки сообщения-заглушки, на случай если отправляются данные которые бот пока
     * не может обработать
     *
     * @param integer $chatID
     * @return ServerResponse
     */
    public static function sendDefaultMessage(int $chatID): void
    {
        Request::sendMessage([
            'chat_id' => $chatID,
            'text' => 'К сожалению пока, я не могу обрабатывать такие сообщения, в дальнейшем планируется добавить обработку фото, видео и голосовых сообщений.'
        ]);
    }
}
